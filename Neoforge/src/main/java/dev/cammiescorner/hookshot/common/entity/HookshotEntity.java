...
public class HookshotEntity extends AbstractArrow {
    ...
    public HookshotEntity(Player owner, Level world) {
        super(HookshotEntities.HOOKSHOT.get(), owner, world, ItemStack.EMPTY, null);
    ...
    private boolean shouldStopExisting(LivingEntity owner) {
        ...
        return HookOwnerAttachment.maybeGet(owner).map(hookOwner -> !hookOwner.hasHook()).orElse(false);
    }
            ...
            if (isPulling) {
                ...
                if (heldStackInfo != null) {
                    ...
                    if (heldStack.getMaxDamage() > 0 && tickCount % 20 == 0) {
                        heldStack.hurtAndBreak(1, owner, LivingEntity.getSlotForHand(hand));
                    }
                }
            }
        }
    ...
    public void remove(RemovalReason reason) {
        if (!level().isClientSide()) {
            HookOwnerAttachment.maybeGet(getOwner()).ifPresent(hookOwner -> {
    ...
    public void onClientRemoval() {
        var owner = getOwner();
        HookOwnerAttachment.maybeGet(owner).ifPresent(hookOwner -> hookOwner.setHasHook(false));
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        stack = ItemStack.parseOptional(registryAccess(), tag.getCompound("hookshotItem"));
    }
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.put("hookshotItem", stack.saveOptional(registryAccess()));
    }
